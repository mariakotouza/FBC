...
logFile = paste0("../",output_folder,slash_for_topics,data_topic,"/log/log_file ",trunc(as.numeric(Sys.time())),"_N_",N,"_",as.numeric(args[3]),"_cores_",cores,".txt")
...
while (!all_leafs) {
  ...
  next_level_cl <- c()
  all_leafs <- T
  templ2=mclapply(cl_at_level,mc.cores = cores, mc.preschedule = TRUE,
                  function(cluster_id) {
  ...
  perlist$temp = llply(templ2,.fun = function(x)  x$permat_m) 
  names(perlist)[length(perlist)] = sprintf('permat_br.%d', cluster_id) # Save the permat with this format
  persimlist$temp = llply(templ2,.fun = function(x)  x$persim)
  names(persimlist)[length(persimlist)] = sprintf('persim_br.%d', cluster_id)
  
  if (!all_leafs) cl_at_level <- unlist(llply(templ2,.fun = function(x)  x$next_level_cl))

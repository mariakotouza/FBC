...
Matrices <- function(list1,let,sim,d,algo,algocol,backcol,backcolj6,logFile){
  tic()
  udata = list1$udata
  br = list1$br
  permat = list1$permat
...
Choice <- function(list2,let,sim,d,algo,algocol,backcol,backcolj6,logFile){
    ...
    if(is.na(str_length(udata[udata$clusters == br,]$AA.JUNCTION[1]))){
      ...
      if(brtemp < clmax){
        ...
        for (i in (br+1):clmax) {
          brtemp = i
          list2 = list("listax" = list2$listax,"ggdf" = list2$ggdf, "dfsum" = list2$dfsum,"list" = list2$list, "listn" = list2$listn, "udata" = list2$udata,"permat"= list2$permat, "persim" = list2$persim, "br" = brtemp, "cl" = list2$cl, "met" = list2$met, "ep"= list2$ep, "clep" = list2$clep,"nn" = list2$nn, "sumper" = list2$sumper, "sumper2" = list2$sumper2, "selected_cell_id" = list2$selected_cell_id, "cel" = list2$cel,"endper" = list2$endper, "last" = list2$last,"progend" = progend, "leaf" = leaf,"leaf2" = leaf2)
          list2 = Matrices(list2,let,sim,d,algo,algocol,backcol,backcolj6,logFile)
        }
      }
      en = toc(quiet = TRUE)
...
BarLev <- function(lev,perlist,Clus,let,flagtic,logFile,jpeg_label){
  ...
  if( is.element(lev,clep) == FALSE){
    ...
    for(i in 1:length(t2)){
      ...
      output <- matrix(unlist(perlist[ar]), ncol = str_length(udata$AA.JUNCTION[1]), byrow = FALSE)
...
Bar_plots_general <- function(g,jpeg_label){
...
findTopicsPerCl <- function(input,numOfTopics){
  if(is.list(input)){
...
computeSimilarityCl<-function(per_matrix){
...
idenlev <- function(lev,clep,Clus,cho,flagtic,logFile){
...
EmPin <- function(Clus,dfsd,flagtic,logFile){
...
###### Graph 
computeDistances <- function(xN,lev,df,Clus,use_only_leafs,sim,altsim){
  ...
  ffg[ffg == 0] = ma
  ffg2[ffg2 == 0] = str_length(udata$AA.JUNCTION[1])
  ffg2sim[ffg2sim == 0] = str_length(udata$AA.JUNCTION[1])
  ffg[lower.tri(ffg)] = NA
  ffg2[lower.tri(ffg2)] = NA
  ffg2sim[lower.tri(ffg2sim)] = NA
  ffg = ffg / ma * 100
  ffg2 = str_length(udata$AA.JUNCTION[1]) - ffg2
  ffg2 = ffg2 / str_length(udata$AA.JUNCTION[1]) * 100
  ffg2sim = str_length(udata$AA.JUNCTION[1]) - ffg2sim
  ffg2sim = ffg2sim / str_length(udata$AA.JUNCTION[1]) * 100
...
Netw <- function(xN,lev,thr,thrt,netyp,df,Clus,use_only_leafs, ffg, ffg2, ffg3, ffg2sim){
...
create_freq_matrix_for_merged_clusters <- function(udata,groups,let,sim){
  #Create frequency matrices 
  mymat = matrix(0,nrow=length(let), ncol=str_length(udata$AA.JUNCTION[1]))
  permat = matrix(0,nrow=length(let) + 1, ncol=str_length(udata$AA.JUNCTION[1]))
  simmat = matrix(0,nrow = max_group_length,ncol =str_length(udata$AA.JUNCTION[1]))

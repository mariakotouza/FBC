# real classes
#actual_classes <- read.csv(paste0(datapath,"/true_classes.txt"), stringsAsFactors = T, header = F)$V1
if (domain != 'AA'){
  labels <- unique(actual_classes)
}

######################### Experiments for thresholds #########################
exp_thresholds=data.frame(matrix(0, nrow = 13, ncol = 16))
...
inner_thr=c(10,10,10,10,10,10,10,0.25,0.5,1,5,1,1)
thr_BS_initial=c(80,83,85,80,83,85,0,0,0,0,0,80,83)
inner_thr=c(10,10,10,10,10,10,10,0.1,0.5,1,5,1,1)
thr_branch_break <- 1

print(thr_branch_breaking)
...
for (exp in 1){
  ...
  threshold1 = thr_branch_breaking[1]
  threshold2 = thr_branch_breaking[2]
  threshold3 = thr_branch_breaking[3]
  threshold4 = thr_branch_breaking[4]
  enthr=T
  levcut=2
  ...
  if(enthr== TRUE){
    ...
    x$clusters <- apply(x[,4:(ncol(x)-1)],1, function(x) ifelse(all(is.na(x)), NA, max(x, na.rm = TRUE)))
    xN <- as.Node(x)
  }
  ...
  #Identity
  exp_thresholds$Identity[exp]=idenlev(num_of_levels,clep,Clus,xN,"Identity",flagtic,logFile)
  ...
  exp_thresholds$num_of_cl[exp]=length(t1)
  
  #bar plot
  BarLev(num_of_levels,clep,xN,perlist,Clus,let,flagtic,logFile,paste0("_thr_BS_initial_",thr_BS_initial[exp],"_thr_inner_thr_",inner_thr[exp]),output_folder,"","",algo,num_of_topics)
...
if (save_rData){
  ...
  save(x,file=paste0(output_folder,slash_for_topics,data_topic,"/RDATA/",algo,"/x","_thrBranch",thr_branch_break,".rData"))
  save(df,file=paste0(output_folder,slash_for_topics,data_topic,"/RDATA/",algo,"/df","_thrBranch",thr_branch_break,".rData"))
  save(Clus,file=paste0(output_folder,slash_for_topics,data_topic,"/RDATA/",algo,"/Clus","_thrBranch",thr_branch_break,".rData"))
  save(clep,file=paste0(output_folder,slash_for_topics,data_topic,"/RDATA/",algo,"/clep","_thrBranch",thr_branch_break,".rData"))
  save(xN,file=paste0(output_folder,slash_for_topics,data_topic,"/RDATA/",algo,"/xN","_thrBranch",thr_branch_break,".rData"))
}
...
#mean values
print(mean(Clus_only_final_level_after_thr$Identity))
...
metrics_per_level_after_thr=EmPin(Clus,clep,xN,dfsd,flagtic,logFile)

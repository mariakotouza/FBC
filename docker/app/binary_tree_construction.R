...
if (load_rData){
  ...
  tic()
  while (progend == FALSE){
    lista = Matrices(listb,let,sim,d,algo,algocol,backcol,backcolj6,logFile,domain,max_group_length)
    progend = lista$progend
    if(progend == TRUE){
      print("ended up here")
      break
    }
    listb = Choice(lista,let,sim,d,algo,algocol,backcol,backcolj6,logFile,domain,max_group_length)
...
for(i in 1:length(ff$branch) ){
  ...
  Clus$BS[ll]=mean(colSums(computeSimilarityCl(perlist[[i]],letter_sim,let)[1:(nrow(perlist[[i]])-1),])) 
}
...
for (i in 1:nrow(Clus)){
  Clus$Topic1[i]=as.numeric(names(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let))[1])
  Clus$Topic2[i]=as.numeric(names(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let))[2])
  Clus$Topic3[i]=as.numeric(names(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let))[3])
  Clus$Topic1_notA[i]=as.numeric(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let)[1])
  Clus$Topic2_notA[i]=as.numeric(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let)[2])
  Clus$Topic3_notA[i]=as.numeric(findTopicsPerCl(AminoCl(Clus$ClusterId[i],lastlist$clep,df,flagtic,logFile),3,let)[3])
  if ((domain != 'AA') & (!is.na(Clus$Topic2[i]))){
